...
// var nameDetails = document.getElementById('userName').value.trim(); // Trim whitespace
const heroTextContent = ' Modhu is Love';
// var finalMessage = nameDetails ? nameDetails + heroTextContent : 'Modhu' + heroTextContent; // Fallback to 'Modhu' if name is empty
typeText(heroText, heroTextContent);
// const userMessage = req.body.message;
//     const defaultMessage = 'Modhu is Love'; // Default message if none provided
//     const heroTextContent = userMessage && userMessage.trim() ? userMessage : defaultMessage; // Use user message or default
//     res.render('custompage', { message: finalMessage });
...
document.addEventListener('click', function showFormOnClick() {
    const customPageForm = document.getElementById('customPageForm');

    // Check if the form is found and then show it
    if (customPageForm) {
    ...
    // Remove this event listener after the first click to avoid multiple triggers
    document.removeEventListener('click', showFormOnClick);
});
